...
- [x] PR #167 리뷰를 반영해 해당 주차의 상위 일정과 주차가 다른 하위 일정도 집계용으로 함께 조회한다.
...
- [x] `_lib/kanban.ts`: `getTasksForWeek`(상위 조회 후 하위 조회로 2회 순차)와 `getCommentsForTasks`(그 뒤 순차 1회)를 `getWeekBoard` 하나로 대체. `weeks`를 inner join으로 걸러 `(year, week_number)`로 직접 조회하므로 weeks 행 조회를 기다리지 않는다
